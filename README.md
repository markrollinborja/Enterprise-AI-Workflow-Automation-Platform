...
- Employee directory: `Employee` + `Department` models (self-referential manager relationship), `/employees` and `/departments` APIs (read for any authenticated user, write restricted to HR/Administrator), a React table view in the dashboard with an HR/Administrator-gated Create Employee form (inline "add new department" option included) and per-row inline Edit — creating an employee here is the only trigger for the onboarding workflow, and there's no API-only backdoor around it
...
- **Notification system (Phase 11, extended post-real-mode-testing)**: `Notification` model + `services/notifications/service.py`'s `notify()`, called from the three points a workflow event matters to a specific person — an approver gets pinged in-app + Slack the moment their approval opens, a submitter gets an in-app notice when their request completes, and an in-app + simulated-email notice if it's rejected (with the rejection reason). A specifically-assigned approver (`manager_approval`, resolved to the employee's actual manager) gets pinged directly; a role-pool approval (IT, Security — no single resolved owner) fans out to every user holding that role instead of notifying no one, which is what it originally did (see [ADR-0014](docs/decisions/0014-role-pool-approval-notification-fanout.md)). Because `_create_approval_request` only fires once a step actually reaches `waiting_approval`, and the engine processes approval steps strictly in sequence, the next approver in a chain is never notified before the prior one has actually decided. One-time notify only in V1, no SLA timers or resend — see the module docstring for why. `GET /notifications` (mine, in-app only) and `POST /notifications/{id}/read` round it out
...
- **Reliability (Phase 13)**: explicit client timeouts on OpenAI (`Settings.openai_timeout_seconds`, default 20s — well under the SDK's own 600s) and MCP calls (`Settings.mcp_call_timeout_seconds`, default 10s), so a hung external call fails fast instead of stalling a poll cycle. A Postgres session-level advisory lock (see [ADR-0013](docs/decisions/0013-advisory-lock-for-workflow-advancement.md)) closes a real race between the worker's poll and an inline API-triggered advance on the same instance. `POST /workflow-instances/{id}/steps/{step_key}/retry` (Administrator-only) lets an admin manually retry a `FAILED` step after fixing the underlying problem — resets the step to `pending`, un-terminals the instance if that failure had taken it down too, and re-runs the engine's normal advance logic inline; `WorkflowStepInstance.retried_by_user_id`/`retried_at` distinguish a manual retry from an automatic backoff retry in the audit timeline. A Retry button on the Workflow Detail page's Steps section (visible on any `failed` step) calls this endpoint and replaces the page's state with the response directly — no second fetch needed, since `advance_workflow` already ran inline server-side
- React + TypeScript + Vite + Tailwind CSS frontend with a working login flow, an in-app notifications panel, an approval inbox, a Request Software Access form (any authenticated user — the real UI trigger for the software-access-request workflow, no API-only path), an employee directory table with Create/Edit, and (Administrator-only) Overview / Workflows / Failed Workflows / Workflow Detail (with per-step manual Retry) / Audit Log dashboard pages, all behind a sidebar-navigation shell
- **Design system**: executes the second half of [ADR-0008](docs/decisions/0008-tailwind-shadcn-frontend.md) (Tailwind now, shadcn/ui layered in once there's a real component inventory) — `src/components/ui/` has hand-written shadcn-pattern primitives (Button, Card, Badge, Input, Label, Textarea, Select, Table, Separator, Avatar) built with `class-variance-authority` + `tailwind-merge`, styled through CSS-variable design tokens in `src/index.css`. Deliberately skips Radix UI: every primitive here is a plain HTML element (native `<select>`, not Radix's Select) since nothing in this app's forms needs Radix's portal/search/virtualization behavior — same "don't add a dependency the app doesn't need" reasoning ADR-0008 already used to defer this in the first place
- Postgres via Docker Compose, with Alembic migrations (`users`, then `departments` + `employees`, then `workflow_definitions` / `workflow_instances` / `workflow_step_instances`, then `workflow_events`, then `approval_requests` / `approval_decisions`, then `applications`, then `access_packages`, then `ai_executions`, then `mcp_tool_executions`, then `workflow_step_instances.external_ref`, then `notifications`, then `workflow_step_instances.retried_by_user_id`/`retried_at`)
...
- Linting (Ruff + MyPy for backend, ESLint for frontend) and a pytest suite covering auth, the employee directory, the workflow state machine, the workflow definition loader/schema, the condition evaluator (including explicit "malicious expression" safety tests), the execution engine (happy path, rejection, retry-and-recover, retries-exhausted, idempotency, worker polling), the approval engine (approver resolution, inbox filtering by role/assignment, approve/reject, authorization, already-decided conflicts), the rules engine (the full risk-classification matrix), the access-request flow (auto-approve, high-risk pause, missing-employee-link, unknown-application, full HTTP round trip), the AI service (confidence-based review routing, graceful fallback on missing key/no catalog/model refusal, the dynamic catalog constraint), the Jira fulfillment webhook (signature verification, idempotency, unknown/non-Done deliveries), the notification system (in-app/Slack/email writes, Slack-failure resilience, no-recipient no-op, engine wiring at all three trigger points), the dashboard/audit endpoints (summary counts and breakdowns, instance list filtering including the failed-only case, instance detail composition, and the composed audit timeline both globally and per-instance), client timeouts (configured value actually reaches the OpenAI/MCP transport), the advisory-lock race fix (a second physical connection genuinely blocks a concurrent advance), and manual retry (a fixed-then-retried step reaches success, a non-failed step 409s, a step whose instance already completed around it is rejected rather than silently stranded, admin-only route gating, and the retry's appearance in the audit timeline) — all against mocked OpenAI and MCP clients, no real API keys needed to run the suite
- **Testing (Phase 14)**: GitHub Actions CI now runs three jobs on every push — `backend` (migrations + ruff + mypy + pytest), `mcp_server` (migrations + ruff + mypy + pytest — previously had real tests that never actually ran in CI), and `frontend` (eslint + build). Both Python suites now report coverage (`pytest-cov`, no enforced minimum — see [docs/testing/testing-strategy.md](docs/testing/testing-strategy.md) for why). That doc also records the two things deliberately not covered (frontend component tests, browser-driven E2E) and the reasoning
- **Real-mode hardening pass**: both V1 workflows were run end-to-end against live Jira, Slack, and Google Calendar (not mocks) to prove the integrations actually work, not just their mocked contracts. This surfaced and fixed four distinct real-world failure modes that mock mode can't catch: (1) `create_jira_task`'s error handling only surfaced Jira's bare status code, not the response body where the actual reason lives — fixed by capturing and surfacing it; (2) `schedule_calendar_event` unconditionally tried to invite attendees, which a bare Google service account (no Workspace domain-wide delegation) can never do regardless of retrying — fixed by dropping the attendee-invite call and folding the intended attendee into the event description instead; (3) Slack notifications originally addressed fictional demo employees by DM, which can't work since they have no real Slack identity — fixed by routing every approval ping to one real, working channel with the recipient named in the message text; (4) the orientation event's start time was computed as naive UTC-as-if-Central, landing at 4 AM local instead of 9 AM — fixed with a real `ZoneInfo("America/Chicago")` conversion. See `docs/troubleshooting.md` for the full incident-by-incident writeup
