...
**Employee** — `id, first_name, last_name, work_email, personal_email (nullable), job_title, department_id (FK), manager_id (FK, self-referential, nullable), employment_type, start_date, status, location, risk_level, created_at, updated_at`. *(Built in Phase 4.)* `access_package_id` is listed in the ERD above but still deliberately **not** a real column, even now that `AccessPackage` exists (Phase 9) — nothing writes a recommendation back onto the employee record yet (no step does that; the recommendation lives in the `recommend_access` step's `output_data` and its `AIExecution` audit row), and nothing reads it until a UI needs to show "current access package" (Phase 12 at the earliest). Same reasoning as `Application.owner_role`'s cut in ADR-0009: don't ship a column nothing uses yet.

**AccessPackage** — `id, name, department_id (FK, nullable), risk_level, included_systems (JSON), description, created_at`. *(Built in Phase 9.)* Seeded reference data (8 rows spanning every department and risk level — see `backend/app/db/seed.py`). `recommend_access_package`'s output is structurally constrained to *this* catalog's current row names via a dynamically-built Pydantic `Literal` (see `services/ai/service.py`) — not just prompted to pick from it.

**AIExecution** — `id, workflow_instance_id (FK), step_instance_id (FK), task_type, input_summary, output_json (nullable), confidence_score (nullable), requires_human_review (nullable), model_used, tokens_used (nullable), status, error_message (nullable), created_at`. *(Built in Phase 9.)* One row per AI call attempt, success or failure — the audit trail Principle 4 asks for ("AI called" / "AI result returned" as their own rows). `input_summary` is a short description of what was sent to the model, not the full prompt — full prompts/responses aren't persisted, see the redaction note below. Nullable on every outcome-dependent field (confidence, output, review flag) because a failed call has none of those.
