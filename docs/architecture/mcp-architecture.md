...
## What real mode actually surfaced

Mock mode proves the *contract* (a tool call returns the right shape and the engine reacts to failure correctly); it can't prove the *integration* actually works against the real API. Running both V1 workflows end-to-end against live Jira, Slack, and Google Calendar (not mocks) surfaced four real failure modes mock mode structurally cannot catch, each with a genuinely different fix — full writeup with exact errors and fixes in [docs/troubleshooting.md](../troubleshooting.md):

1. **Swallowed error detail.** `create_jira_task`'s original `httpx.HTTPStatusError` handling surfaced only the bare status code — Jira's actual rejection reason lives in the response body, which was never read. A retryable-looking failure was, in reality, permanently misconfigured credentials with no way to tell from the error message.
2. **A permanent, non-retryable platform limitation.** `schedule_calendar_event` unconditionally tried to invite attendees. A bare Google service account (no paid Workspace domain-wide delegation) can never do this — retrying is not a fix, only removing the attendee-invite call is. This is the clearest example in this project of Principle 1 (enterprise realism, not enterprise scale) actually constraining an implementation choice, not just informing a design doc.
3. **An addressing scheme that assumed real identities.** Slack notifications originally addressed fictional demo employees by DM, which requires a real Slack user ID and real workspace membership neither of them has. Routing to one real, working channel with the recipient named in the message text is a demo-appropriate substitute, not a "real" per-user DM system — documented as a known V1 limitation, not hidden.
4. **A silent notification gap that mock-mode tests never exercised the visible cost of.** Role-pool approvals (IT, Security) originally created zero notifications by design (see [ADR-0014](../decisions/0014-role-pool-approval-notification-fanout.md)) — technically correct, but a chain that stops pinging anyone after step one reads as broken in a live demo even though the backend state is right.

None of these were caught by the mocked test suite, and none of them could have been — they're all specific to the real external system's actual behavior, not to this codebase's logic. That's the argument for running a real-mode pass at all before calling an integration "done," not just a nice-to-have.
